...
//            Locale.setDefault(locale);
    ...
    private void SetUpCalndar() {
        ...
        //CollectEventMonth();
        calendarViewGridAdapter = new CalendarView_GridAdapter( context, dates, calendar, eventsList, test );
        gridView.setAdapter( calendarViewGridAdapter );

       calendarViewGridAdapter.setOnClick( new CalendarView_GridAdapter.onClickListener() {
           @Override
...
//                tv2.setText( position.toString() );
               ...
               for (int i = 0;  i< CalendarView_GridAdapter.events.size(); i++){
                   if(ConvertStringToDate(CalendarView_GridAdapter.events.get( i ).date).equals( ConvertStringToDate(position) )){
//                        Toast.makeText( getContext(),CalendarView_GridAdapter.events.get( i ).getEvent() , Toast.LENGTH_SHORT).show();
                       //System.out.println("EV "+ CalendarView_GridAdapter.events.get( i ).getEvent());
                       dateFromEventsMore2.add( CalendarView_GridAdapter.events.get( i ).getEvent() );
...
//                System.out.println( dateFromEventsMore2 );
        ...
        calendarViewGridAdapter.setOnLongClick( new CalendarView_GridAdapter.onLongClickListener() {
            @Override
            public void onItemClick(String position) {
