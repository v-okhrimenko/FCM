...
public class DatePicker_GridAdapter extends ArrayAdapter  {
    ...
    public DatePicker_GridAdapter(@NonNull Context context, List<Date> dates, Calendar currentDate, List<DatePicker_Events> events, ArrayList<String> test) {
...
//        new Thread(run).start();
        ...
        for (int two = 0; two< Calendar_main_activity.two_or_more_event.size(); two++){
            Calendar calpovtor = Calendar.getInstance();
            Date twoDate = ConvertStringToDate( Calendar_main_activity.two_or_more_event.get( two ) );
            calpovtor.set( Calendar.DAY_OF_MONTH, twoDate.getDate() );
            calpovtor.set( Calendar.MONTH, ConvertStringToDate( Calendar_main_activity.two_or_more_event.get( two ) ).getMonth() );
            calpovtor.set( Calendar.YEAR, ConvertStringToDate( Calendar_main_activity.two_or_more_event.get( two ) ).getYear()+1900 );
        ...
        if (Calendar_main_activity.two_or_more_event.size()<1){
            LinearLayout two_or_more_event = finalView.findViewById(R.id.calendar_if_many_dots );
    ...
    public void setOnLongClick(CalendarView_GridAdapter.onLongClickListener listener) {
